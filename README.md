...
**최종 업데이트**: 2026-02-11  
**최신 작업 반영**: **Merge·Reflow·Typecheck/Lint·Tide Phase1·고급 (2026-02-11)** — Reflow: **dependency cascade** preview 경로 통일(`previewScheduleReflow`). **Tide 고급**: SAFE 2h 연속 슬롯 자동 추천(`findNearestSafeSlot`), DANGER 드래그 시 What-if(+1/+2일) 제안(`buildShiftDayWhatIf`). TideOverlayGantt 가이던스 패널·Apply nearest SAFE/+1d/+2d; 비-DANGER 시 **View guidance** 버튼으로 수동 오픈. [CHANGELOG.md](CHANGELOG.md), [docs/TYPECHECK_AND_LINT_FAILURES.md](docs/TYPECHECK_AND_LINT_FAILURES.md).
