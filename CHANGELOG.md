...
- **Weather Overlay 구현 완료 (2026-02-04)**: Canvas 배경 레이어 (z-0)로 NO_GO/NEAR_LIMIT 날씨 시각화. 신규 파일 3개 (`weather-overlay.ts`, `WeatherOverlay.tsx`, `weather-overlay.test.ts`). Opacity 슬라이더 (5-40%, Gantt Legend 통합), UI 토글 (🌦️/🌤️), Range culling (viewStart/viewEnd), RAF throttle (10fps), DPI scaling (max 2x). 테스트 2/2 passed ✅.
