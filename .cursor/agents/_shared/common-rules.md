...
## 11. 문서 일관성 (Documentation Consistency)

**적용 대상**: 문서를 생성·수정하는 모든 에이전트 (tr-planner, tr-implementer, tr-dashboard-patch, docops-autopilot 등).

### 11.1 문서 SSOT·상호 Ref
- **핵심 문서**: README.md, docs/LAYOUT.md, docs/SYSTEM_ARCHITECTURE.md, docs/WORK_LOG_*.md, docs/BUGFIX_APPLIED_*.md, docs/INDEX.md
- **규칙**: 위 문서를 수정할 때 **상호 Ref**(상단/하단 Refs 섹션)를 일치시킨다. 한쪽만 갱신하지 않는다.
- **최종 업데이트/최신 작업 반영**: WORK_LOG/BUGFIX 반영 시 해당 문서의 "최종 업데이트" 날짜 및 Refs 링크를 함께 갱신한다.

### 11.2 본문 내용 반영
- **규칙**: 메타(날짜/Refs)만이 아니라 **본문(섹션·문구·설명)** 도 최신 작업(Phase/Bugfix)에 맞게 수정한다.
- **소스**: WORK_LOG_*.md, BUGFIX_APPLIED_*.md, docs/plan/*. UI/코드 변경 시 해당 문서의 해당 섹션 문구를 실제와 일치시킨다.
- **예**: StoryHeader WHERE/WHEN/WHAT 제거 → 문서 전체에서 Location/Schedule/Verification, Map/Timeline으로 통일. DatePicker UTC, View 버튼, Trip fallback 등 반영.

### 11.3 DocOps 연동
- **문서정리/문서 만들기** 시: [docops-autopilot](../../skills/docops-doc-manager/SKILL.md) 규칙 준수. LATEST WORK REFLECT → 본문 반영 → REF → (필요 시) Mermaid 그래프 → Verify.
- **규칙 경로**: `.cursor/skills/docops-doc-manager/references/docops-latest-work.md`, `docops-rules.md`

### 11.4 커밋
- 문서 변경은 **docs: <description>** 커밋으로 분리. 파이프라인 실패 시 커밋 중단.
