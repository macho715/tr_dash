...
| 에이전트 | 역할 | 모드 | 트리거 |
|----------|------|------|--------|
| **innovation-scout** | 프로젝트 현황 분석 + 외부 리서치 + 아이디어 검증 | readonly | "아이디어", "트렌드", "best practice", "개선" |

**실행 순서**: innovation-scout → (APPLICABLE 판정 시) tr-planner → tr-implementer → tr-verifier
...
사용자 요청
    ...
    │
    ├─ "아이디어" / "트렌드" / "best practice" / "개선" / "리서치" 포함?
    │   └─ YES → innovation-scout
    │
    └─ 불명확 → 사용자에게 명확화 요청
```
...
### 4.4 혁신/개선 사이클
```yaml
pipeline: innovation-cycle
steps:
  - agent: innovation-scout
    output: docs/plan/innovation-report.md
    tools: [web_search, web_fetch]
  - agent: tr-planner
    depends_on: innovation-scout
    condition: "idea.feasibility == APPLICABLE"
    output: 계획에 아이디어 반영
  - agent: tr-implementer
    depends_on: tr-planner
    output: 코드 변경
...
→ 자동으로 planner → implementer → verifier 실행
