...
**생성일**: 2026-02-02  
**갱신일**: 2026-02-03 (tr-planner: 커맨드 탐지 갱신, 참조 정리)  
**참조**: GANTTPATCH.MD, GANTTPATCH1.MD, GANTTPATCH2.MD, **GANTTPATCH4.MD**, vis-timeline-gantt, AGENTS.md, patch.md, LAYOUT.md, [innovation-report.md](./innovation-report.md)
...
실제 구현: `components/gantt/VisTimelineGantt.tsx` (GanttSection에서 사용).
...
> `scripts/detect_project_commands.py` 실행 시 동일 출력 기대. 아래는 package.json 기준 정리.

```json
{
  "workspace_root": "tr_dashboard-main",
  ...
  "scripts": {
    ...
    "test": "vitest",
    "test:run": "vitest run",
    "schema:smoke": "tsx scripts/schema-smoke.ts",
    "validate:ssot": "python scripts/validate_optionc.py tests/fixtures/option_c_baseline.json",
    "sync:schedule": "python scripts/sync_schedule_to_dashboard.py",
    "sync:tide": "python scripts/sync_tide_to_dashboard.py",
    "sync:agent": "python scripts/sync_agent_to_dashboard.py"
  }
}
```
...
- [ ] lint/typecheck/test/build PASS — 현재 상태: [tr-dashboard-verification-report.md](./tr-dashboard-verification-report.md) 참고 (Lint/Typecheck FAIL)
...
- **patchmain 14-item 수정** — [plan_patchmain_14.md](./plan_patchmain_14.md) (P0~P2 UX/품질, Tidy First)
...
- [innovation-report.md](./innovation-report.md) — 프로젝트 개선 아이디어(P1/P2) 및 적용 가능성
...
## 8) 백로그 (선택 — innovation-report 기반)

| 우선순위 | 아이디어 | 산출물/파일 | 검증 |
|----------|----------|-------------|------|
| P1 | Data Freshness Indicator | components/ui/data-freshness-indicator.tsx | StoryHeader/ControlBar 연동 |
| P1 | Collision 탐지 최적화 (O(n log n)) | lib/utils/detect-resource-conflicts.ts | 기존 결과 동일성 테스트 |
| P2 | PWA + 오프라인 Field Mode | next-pwa, Service Worker | 오프라인 캐시 + Evidence 큐 |
| P2 | WebSocket 실시간 업데이트 | lib/websocket/, /api/ws/ | 활동/GPS 스트림 |
| DoD | lint/typecheck/test/build PASS | — | 매 Task 후 실행 |
| DoD | validate_optionc.py (option_c 변경 시) | scripts/validate_optionc.py | CONTRACT 모드 |
