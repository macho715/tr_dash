아래는 **tr_dash(TR Dashboard)** 쪽에 **convert/.cursor의 `agi-schedule-updater` “의도(스케줄 업데이트 자동화)”를 포팅**하면서, **대시보드 UI에 “검색+업데이트” 박스를 추가**해 **동일 기능을 클릭 몇 번으로 실행**되도록 만든 적용안입니다.
...
* tr_dash에도 `.cursor/rules`, `.cursor/commands` 구조가 이미 있으므로 ([GitHub][1])
...
description: AGI TR schedule update workflow (preview→apply, export patch) for tr_dash
---
...
```md
---
description: Guardrails for AGI schedule updates (tr_dash)
globs:
  - "app/**/*.tsx"
  - "components/**/*.tsx"
  - "lib/**/*.ts"
...
* convert는 “업무 자동화 모듈 집합(AGI 포함)” 성격이고 ([GitHub][4])
  tr_dash는 이미 **스케줄 재계산 엔진 + Preview + 자원충돌 탐지 흐름**을 갖고 있습니다. ([GitHub][1])
