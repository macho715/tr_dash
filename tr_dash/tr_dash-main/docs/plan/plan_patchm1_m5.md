...
**Evidence 정책 정리**: Upload = 파일 업로드 제거, Evidence 입력은 모달(링크/시뮬레이션) 유지. (patchm2·tr-dashboard-plan-patch4 Phase 8·실제 구현과 일치)
...
**실제 통과 현황**: `docs/plan/tr-dashboard-verification-report.md` 참고 (전체 170 tests). 본 계획 Tests 항목과의 1:1 매핑은 해당 리포트·DoD 조합으로 추적.
