...
### 11. T1: Gantt/Map 지연 로딩·스켈레톤·근접 선로딩·fail-soft

- **4축**: (1) 지연 로딩 경계 단일화 (2) 스켈레톤 (3) 근접 선로딩 (4) fail-soft fallback.
- **Map**: MapPanelWrapper — MapPanel 정적 import, `preloadMapPanel()`, MapPanelSkeleton, MapListFallback. tr-three-column-layout `id="map"`.
- **Gantt**: loadVisTimelineGantt 분리, `preloadVisTimelineGantt()`, GanttSkeleton, GanttListFallback.
- **lib/perf/use-near-viewport-preload.ts**: viewport 근접 시 map/gantt preload. page에서 2회 훅 호출 + ErrorBoundary fallback.
- **테스트**: MapPanelWrapper.loading, gantt-loading, use-near-viewport-preload, widget-failsoft-fallback. tsc·build 통과.

### 12. FilterDrawer 모듈

- `components/dashboard/FilterDrawer.tsx` — named/default export 둘 다 제공, 모바일 UX용 필터 드로어.

### 13. AI LLM 타임아웃 · Fallback · 컨텍스트 축소
