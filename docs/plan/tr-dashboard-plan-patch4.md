...
2. **2-column Layout**: 좌 Map(Where)+Detail(State/Risk), 우 Timeline/Gantt(When/What) (tr-three-column-layout)
...
- [x] **T8.2** Evidence tab (`components/evidence/EvidenceTab.tsx`)
  ...
  - **Evidence 정책**: 파일 업로드 제거, Evidence 입력은 모달(링크/시뮬레이션)만 유지 (plan_patchm1_m2·plan_patchm1_m5와 동일).
...
**참고**: 실제 레포는 루트 `lib/`, `components/`, `app/` 및 `src/lib/` 혼합 구조. 아래 트리는 계약/역할 기준 표기이며, 경로는 프로젝트 실제 구조에 맞춰 조정됨.
