...
**현재 상태**: **마이그레이션 보류.** 사용자/프로젝트 결정으로 option_c는 **AGI 형식 유지** (WORK_LOG_20260202, tr-dashboard-plan-patch4 참고). 본 문서는 추후 Contract v0.8.0 전환 시 참고용으로 보존. `scripts/migrate-to-v0.8.0.ts` 존재 시 실행/검증은 재개 전까지 수행하지 않음. Phase 7 (DetailPanel, WhyPanel, ReflowPreviewPanel) 완료.
...
**보류. 재개 시 Phase 1부터.**  
현재는 마이그레이션을 실행하지 않음. 참고용 보존.

<details>
<summary>재개 시 실행 예시 (접기)</summary>

```bash
# Run migration (스크립트 존재 시)
pnpm tsx scripts/migrate-to-v0.8.0.ts

# Validate output (CONTRACT 모드 = v0.8.0 형식 검증; AGI 형식은 기본 모드)
# tests/fixtures/option_c_baseline.json 이 AGI인지 v0.8.0인지에 따라 모드 선택
python scripts/validate_optionc.py tests/fixtures/option_c_baseline.json
# v0.8.0 출력 검증: VALIDATION_MODE=CONTRACT python scripts/validate_optionc.py <output_path>
```
...
## Validation / Fixtures 참고

- **validate_optionc.py**: CONTRACT 모드 = v0.8.0 스키마 검증, 기본 모드 = AGI 형식 검증. 재개 전에 스크립트가 두 모드를 지원하는지 확인 권장.
- **tests/fixtures/option_c_baseline.json**: 현재 baseline이 AGI 형식인지 v0.8.0 형식인지 문서/코드에 한 줄이라도 명시해 두면 혼동 감소.

---

## References

- contract-optionc-v0.8.0.md (`.cursor/skills/tr-dashboard-autopilot/references/`)
- runbook-state-reflow-collision.md (`.cursor/skills/tr-dashboard-autopilot/references/`)
- tr-dashboard-plan.md
- docs/plan/tr-dashboard-verification-report.md
- WORK_LOG_20260202.md (마이그레이션 취소 반영)
- tr-dashboard-plan-patch4.md (Contract v0.8.0 ⏸️ 마이그레이션 취소)
